...
export default function ProductDrawer({ open, product, products, onClose, onNavigate }: ProductDrawerProps) {
      ...
      {open && product && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <motion.div
            ...
            style={{
              width: 'min(max(45vw, 640px), 920px)',
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-7 py-5" style={{ scrollbarWidth: 'thin', scrollbarColor: 'var(--text-tertiary) transparent' }}>
              <div className="space-y-7" style={{ paddingBottom: 28 }}>
                ...
                {tab === 'sku' && <SkuTab product={product} />}
...
function SkuTab({ product }: { product: Product }) {
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border-default)' }}>
      <table className="w-full text-left">
        <thead>
          <tr style={{ backgroundColor: 'var(--bg-subtle)' }}>
            {['主图', 'SKU 编码', '颜色', '英文名', '中文译名', '售价', '成本价', '库存'].map((h, i) => (
              <th key={i} className={`px-3 py-3 text-[10px] font-medium ${i === 0 ? 'w-12' : ''}`} style={{ color: 'var(--text-tertiary)' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {product.skus.map(sku => (
            <tr key={sku.code} style={{ borderTop: '1px solid var(--border-soft)' }}>
              <td className="px-3 py-2.5">
                <div className="w-9 h-9 rounded-md overflow-hidden" style={{ backgroundColor: 'var(--bg-subtle)' }}>
                  <img src={`https://placehold.co/36x36/F6F9FC/888?text=${sku.color}`} alt="" className="w-full h-full object-cover" />
                </div>
              </td>
              <td className="px-3 py-2.5 text-[11px] font-mono" style={{ color: 'var(--text-tertiary)' }}>{sku.code}</td>
              <td className="px-3 py-2.5 text-[12px]" style={{ color: 'var(--text-primary)' }}>{sku.color}</td>
              <td className="px-3 py-2.5 text-[11px]" style={{ color: 'var(--text-tertiary)' }}>{sku.color}</td>
              <td className="px-3 py-2.5"><GlassInput defaultValue={sku.color} /></td>
              <td className="px-3 py-2.5"><GlassInput defaultValue={`${sku.price}`} prefix="¥" /></td>
              <td className="px-3 py-2.5"><GlassInput defaultValue={`${sku.cost}`} prefix="¥" /></td>
              <td className="px-3 py-2.5"><GlassInput defaultValue={`${sku.stock}`} /></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function ImagesTab({ product }: { product: Product }) {
  ...
  const filtered = segment === '尺寸图' || segment === '视频' ? [] : all.filter(i => i.type === segment)
...
/* ================================================================
   ...
   ================================================================ */
function MediaTile({ index, url, type, onClick }: { index: number; url: string; type: string; onClick: () => void }) {
  return (
    <div
      className="group relative rounded-[16px] overflow-hidden cursor-pointer transition-all duration-220 hover:-translate-y-0.5"
      style={{ aspectRatio: '1', backgroundColor: 'var(--bg-subtle)' }}
      onClick={onClick}
    >
  ...
  return (
    <div className="relative">
      ...
      <input defaultValue={defaultValue}
        className={`w-full h-8 rounded-md text-[11px] outline-none transition-all duration-220 ${prefix ? 'pl-5' : 'px-2.5'}`}
        style={{ backgroundColor: 'var(--input)', color: 'var(--text-primary)', border: 'none' }}
        onFocus={e => { e.currentTarget.style.backgroundColor = 'var(--input-focus)'; e.currentTarget.style.boxShadow = 'var(--accent-glow)' }}
        onBlur={e => { e.currentTarget.style.backgroundColor = 'var(--input)'; e.currentTarget.style.boxShadow = 'none' }} />
    </div>
  )
}
